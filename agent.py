...
def evaluate_todays_signals_on_5min_data():
    """
    EVENING MODE: Načte dnešní signály z JSON a vyhodnotí je proti 5min datům.
    UPRAVENO: Používá TRAILING STOP LOSS s polovičním posuvem.
    Vrací updated signály s výsledky a profitem.
    """
    print(f"\n{'='*70}")
    print(f"📊 EVENING MODE: VYHODNOCENÍ DNEŠNÍCH SIGNÁLŮ (TRAILING SL)")
    print(f"{'='*70}\n")
    ...
    # 3. Vyhodnoť každý signál s TRAILING STOP LOSS
    print(f"\n📊 Vyhodnocuji signály s TRAILING STOP LOSS (posun o půlku změny)...")
    evaluated_trades = []
    ...
    for strategy, signals in todays_signals.items():
        ...
        for sig in signals:
            ...
            try:
                ...
                if len(daily_data) >= 20:
                    avg_range = float((daily_data['High'] - daily_data['Low']).rolling(20).mean().iloc[-1])
                    initial_sl_distance = sl_factor * avg_range
                else:
                    # Fallback: 2% range
                    initial_sl_distance = entry_price * 0.02
            except:
                initial_sl_distance = entry_price * 0.02
            
            # TRAILING STOP LOSS LOGIKA (NO LOOK-AHEAD BIAS)
            hit_sl = False
            exit_price = None
            
            if action == 'Long':
                # Počáteční stop loss
                trailing_sl = entry_price - initial_sl_distance
                highest_price = entry_price
                
                # Procházíme každý 5min bar
                for idx, bar in todays_bars.iterrows():
                    bar_high = float(bar['High'].iloc[0] if hasattr(bar['High'], 'iloc') else bar['High'])
                    bar_low = float(bar['Low'].iloc[0] if hasattr(bar['Low'], 'iloc') else bar['Low'])
                    
                    # PRVNÍ: Zkontroluj, zda jsme trefili trailing SL (používáme SL z předchozího baru)
                    if bar_low <= trailing_sl:
                        exit_price = trailing_sl
                        hit_sl = True
                        break
                    
                    # DRUHÉ: Aktualizuj trailing SL pro DALŠÍ bar (pokud jsme nebyli stopnuti)
                    if bar_high > highest_price:
                        price_increase = bar_high - highest_price
                        # Posun trailing SL o POLOVINU změny (růstu) - platí až od dalšího baru
                        trailing_sl += price_increase * 0.5
                        highest_price = bar_high
                
                # Pokud nebyl SL aktivován, exit na konci dne
                if not hit_sl:
                    exit_bar = todays_bars.iloc[-1]
                    exit_price = float(exit_bar['Close'].iloc[0] if hasattr(exit_bar['Close'], 'iloc') else exit_bar['Close'])
                ...
                for idx, bar in todays_bars.iterrows():
                    ...
                    if bar_low < lowest_price:
                        price_decrease = lowest_price - bar_low
                        # Posun trailing SL o POLOVINU změny (poklesu) - platí až od dalšího baru
                        trailing_sl -= price_decrease * 0.5
            ...
            status = "🛑 Trailing SL Hit" if hit_sl else "✅ Closed"
            print(f"   {ticker:6s} {action:5s}: Entry=${entry_price:.2f} Exit=${exit_price:.2f} "
                  f"P&L=${net_pnl:>+8.2f} {status}")
...
def optimize_sl_for_ticker_strategy(sig_df, mode_strat, ticker):
    """Grid search pro optimální SL."""
    best_sl = 0.5
    best_z = -999
    best_metrics = {}
    ...
    for sl in SL_GRID:
        ...
        for idx, row in sig_df.iterrows():
            ...
            pnl, _ = simulate_trade_with_sl(row, side, sl, COMMISSION_PCT)
            profits.append(pnl)
        
        z = calculate_z_score(profits)
    ...
    return best_sl, best_metrics

def run_backtest_60d(ticker_data, optimized_sl, ticker_performance):
    """Backtest pro posledních 60 dní - používá TOP 3 akcie podle seřazení pro každou strategii, max 3 obchody za den."""
    print(f"{'='*70}")
    print(f"📈 BACKTEST NA POSLEDNÍCH 60 DNECH (TOP 3 AKCIE, MAX 3 OBCHODY/DEN)")
    print(f"{'='*70}\n")
    
    results_by_strategy = {m: [] for m in ['A', 'B', 'V', 'M']}
    
    for mode_strat in ['A', 'B', 'V', 'M']:
        allowed_tickers = STRATEGY_TICKER_GROUPS[mode_strat]
        
        # Získej všechna data pro tuto strategii
        strategy_data = {}
        for t in allowed_tickers:
            if t not in ticker_data:
                continue
            strategy_data[t] = ticker_data[t].iloc[-BACKTEST_DAYS:]
        
        if not strategy_data:
            continue
        
        # Získej všechna unikátní data v backtestovacím období
        all_dates = set()
        for df in strategy_data.values():
            all_dates.update(df.index)
        all_dates = sorted(all_dates)
        
        # Pro každý den: najdi signály, seřaď podle score, vyber TOP 3
        trades_per_day = {}
        for date in all_dates:
            ...
            for t in strategy_data:
                ...
                # Zkontroluj zda máme data pro tento den
                if date not in df.index:
                    continue
                
                row = df.loc[date]
                
                # Zkontroluj podmínky signálu
                is_signal = False
                score = 0
                ...
                if mode_strat == 'A':
                    ...
                    if dist_h < 0.4:
                        is_signal = True
                        score = ticker_performance[mode_strat].get(t, 0)
                elif mode_strat == 'B':
            ...
            # Seřaď podle score a vyber TOP 3
            top_3 = sorted(candidates, key=lambda x: x['score'], reverse=True)[:3]
            
            # Proveď obchody pro TOP 3
            for candidate in top_3:
                t = candidate['ticker']
                row = candidate['row']
                sl = optimized_sl[mode_strat].get(t, 0.5)
                ...
                pnl, hit_sl = simulate_trade_with_sl(row, side, sl, COMMISSION_PCT)
                
                results_by_strategy[mode_strat].append({
    ...
    summary = {}
    for mode in ['A', 'B', 'V', 'M']:
        trades = results_by_strategy[mode]
        if trades:
            profits = [t['profit'] for t in trades]
            summary[mode] = {
                'num_trades': len(trades),
                ...
                'avg_profit': round(np.mean(profits), 2),
                'win_rate': round(100 * sum(1 for p in profits if p > 0) / len(profits), 1)
            }
            print(f"Strategie {mode}: {len(trades)} obchodů | P&L=${sum(profits):+.2f} | Win={summary[mode]['win_rate']:.1f}%")
    
    return {'summary': summary, 'trades': results_by_strategy}

def generate_signals_for_tomorrow(ticker_data, optimized_sl, ticker_performance):
    """Generuje signály pro následující trading day."""
    print(f"\n{'='*70}")
    print(f"🎯 GENEROVÁNÍ SIGNÁLŮ PRO ZÍTŘEK")
    print(f"{'='*70}")
...
def run_agent():
    ...
    1. Vyhodnotí dnešní signály na 5min datech s TRAILING STOP LOSS
    2. Uloží výsledky do CSV
    3. Přepočítá SL optimization
    4. Vygeneruje signály pro zítřek
    5. Uloží všechny soubory
    """
    print(f"🚀 TRADING AGENT - EVENING MODE (TRAILING SL)")
    print(f"📅 Datum: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
    
    # KROK 1: Vyhodnoť dnešní signály na 5min datech s TRAILING SL
